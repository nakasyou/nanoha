...
const Settings = component$<{
  ...
  return <div class="w-full h-dvh fixed z-20 top-0 left-0 bg-[#000a] grid items-center p-3">
    <div class="bg-surface rounded-lg p-2 border border-outline">
                ...
                screenState.note!.notes.map((note) => {
                  ...
                  return <label key={note.id}>
                    <div class="flex gap-2">
                      <input type="checkbox" checked={screenState.rangeNotes.has(note.id)} onChange$={(evt) => {
      ...
      <div>
        <button class="filled-button" type="button" onClick$={() => {
          props.onClose$()
...
export const Navbar = component$(() => {
  ...
  return (
    <>
      {
        isSettingsOpened.value && <Settings onClose$={() => {
