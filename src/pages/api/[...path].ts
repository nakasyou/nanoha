...
import { type Credentials, credentialsSchema, fetchUserInfo, refreshAccessToken } from '../../utils/google-oauth'
...
const googleRoutes = app.basePath('/google')
  .use('*', async (c, next) => {
    ...
    const newCredentials = await refreshAccessToken(credentials.output.refresh_token || '')
  ...
  .get('/get-avater', async c => {
    ...
    return c.body(await fetch(userInfo.data.picture).then(res => res.arrayBuffer()))
  })
